...
pub const GatewayIntents = packed struct {
    pub fn toRaw(self: GatewayIntents) u28 {
        return @as(u28, @bitCast(self));
    }

    pub fn fromRaw(raw: u28) GatewayIntents {
        return @as(Intents, @bitCast(raw));
    }

    pub fn jsonStringify(self: GatewayIntents, options: std.json.StringifyOptions, writer: anytype) !void {
        _ = options;
...
pub const Application = struct {
    ...
    integration_types_config: ?Partial(AutoArrayHashMap(ApplicationIntegrationType, ApplicationIntegrationTypeConfiguration)),
...
pub const ApplicationRoleConnection = struct {
    ...
    metadata: AutoArrayHashMap([]const u8, []const u8),
};
...
pub const InteractionData = struct {
    ...
    resolved: ?struct {
        ...
        messages: ?AutoArrayHashMap([]const u8, Message),
        /// The Ids and User objects
        users: ?AutoArrayHashMap([]const u8, User),
        /// The Ids and partial Member objects
        members: ?AutoArrayHashMap([]const u8, Omit(InteractionMember, .{ "user", "deaf", "mute" })),
        ...
        attachments: AutoArrayHashMap([]const u8, Attachment),
    },
